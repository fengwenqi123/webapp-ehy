...
<!--    </div>-->
    <div class="content">
      ...
      <div class="submit">
        <van-button class="button" size="normal" @click="validate(submitAndBack)">保存并返回</van-button>
        <van-button class="button" type="info" size="normal" @click="validate(submitAndNew)">保存并新建</van-button>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submitAndBack() {
        this.$store.commit('set_boxInfo', this.boxInfo)
        this.$router.go(-1)
      },
      submitAndNew() {
        this.$store.commit('set_boxInfo', this.boxInfo)
