  ...
  <div class="container">
    ...
    <div class="register"
      ...
      <ul>
        ...
        <li>
          <span>起始港</span>
          <span @click="startPort()">{{startPortText}}</span>
        </li>
        <li>
          <span>目的港</span>
...
export default {
  ...
  methods: {
    ...
    guoZhaInfoByBaoGangId() {
      getGuoZhaInfoByBaoGangId(this.id).then(response => {
        this.serviceArea = response.transferPoints
        this.portItems = response.ports
        this.goodsItems = response.goodTypes
      }).catch(error => {
        console.log(error)
      })
    },
    basicInfo() {
      getBasicInfo(this.shipName).then(response => {
        this.serviceArea = response.transferPoints
        this.portItems = response.ports
        this.goodsItems = response.goodTypes
      }).catch(error => {
    ...
    portSumitClick() {
      ...
      dataObj.goods = arr
      console.log(dataObj)
      // const dataObj2 = JSON.stringify(dataObj)
      // const paramsBox = encodeURIComponent(dataObj2)
      Toast.loading({
        mask: true,
        message: '加载中...',
        duration: 0
      })
      postBaoGang(dataObj).then(response => {
        Toast.clear()
        localStorage.gateSowId = response.obj.id
        localStorage.gateState = '1'
    ...
    shipInfo() {
      getShipInfo(this.shipName).then(response => {
        ...
        this.typeText = response.obj.cbzl
        this.shipTypeId = response.obj.cbzldm
        const shipType = this.shipTypeId.slice(0, 2)
        if (shipType === '03') {
          this.dangerShip = '是'
          this.dangerId = 1
        } else {
        ...
        this.shipName = response.obj.zwcm
        this.input = {
          water: '',
          freeboard: '',
          height: '',
          tonnage: response.obj.jdw || '',
          DWT: response.obj.ckzzd || '',
          horsepower: response.obj.zjzgl || '',
          seat: response.obj.ckdezj || '',
          length: response.obj.cz || '',
          phone: ''
        }
      }).catch(error => {
        console.log(error)
